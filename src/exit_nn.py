...
class ExitNN(object):
    ...
    def getFromCollection(self):
        """
        Retrieves nodes from the TensorFlow collection.
        Returns a dictionary mapping node names to their corresponding TensorFlow nodes.
        If the collection is empty or nodes don't exist, creates default nodes.
        """
        try:
            # 尝试从集合中获取已经存在的节点
            nodes = {}
            ...
            # 必要节点的名称列表
            essential_nodes = ['x', 'y', 'output', 'optimizer', 'cost']
            
            # 检查每个节点是否存在
            for node_name in essential_nodes:
                collection = tf.get_collection(node_name)
                if collection and len(collection) > 0:
                    nodes[node_name] = collection[0]
                    print(f"Found node '{node_name}' in collection")
                else:
                    print(f"Warning: Node '{node_name}' not found in collection")
            
            # 检查是否成功恢复了关键节点
            if 'x' in nodes and 'output' in nodes:
                return nodes
            
            # 如果没有恢复关键节点，尝试从图中获取这些节点
            ...
            if 'x' not in nodes:
                try:
                    ...
                    print("Could not find 'x' node by name")
            ...
            if 'output' not in nodes:
                try:
                    ...
                    print("Could not find 'output' node by name")
            
            # 返回找到的节点，即使不完整
            return nodes
            
        except Exception as e:
            print(f"Error in getFromCollection: {e}")
            # 如果出错，返回空字典
            return {}
